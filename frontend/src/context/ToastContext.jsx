    ...
    <ToastContext.Provider value={{ addToast, removeToast }}>
      ...
      <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-3 max-w-sm w-full px-4 pointer-events-none">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`pointer-events-auto flex items-start gap-3 rounded-2xl p-4 shadow-xl border backdrop-blur-md transition-all duration-300 animate-slide-up ${
              toast.type === 'success'
                ? 'bg-white text-slate-900 border-emerald-500/40 shadow-slate-900/10'
                : toast.type === 'error'
                ? 'bg-white text-slate-900 border-red-500/50 shadow-slate-900/10'
                : toast.type === 'wishlist'
                ? 'bg-white text-slate-900 border-rose-500/40 shadow-slate-900/10'
          ...
          >
            <div className="shrink-0 mt-0.5">
              ...
              {toast.type === 'wishlist' && <Heart className="text-rose-600 fill-rose-600" size={20} />}
              {toast.type === 'info' && <Info className="text-blue-600" size={20} />}
            </div>
            <div className="flex-1 min-w-0">
              {toast.title && <h4 className="text-sm font-bold text-slate-900 leading-snug">{toast.title}</h4>}
              ...
              {toast.action && (
                <div className="mt-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs font-bold text-red-600 hover:text-red-700 underline underline-offset-2"
                  >
                    {toast.action.label}
                  </button>
                </div>
